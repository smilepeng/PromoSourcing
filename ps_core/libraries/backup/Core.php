...
class Core {
	...
	{	
		...
		$this->groupID = ($this->CI->session->userdata('groupID'))   ? 0 : $this->CI->session->userdata('groupID') ;	 //Edited By Smy
	...
	function generate_page($pageID, $admin = FALSE, $templateID = '')
	{
		...
		//log_message('error', 'mkdn body:'. print_r($mkdnBody, true));
		// parse for blocks
		//preg_match_all('/block([a-z0-9\-_]+)/i', $page['body'], $blocks); //Edited by smy
		preg_match_all('/{block([a-z0-9\-_]+)}/i', $page['body'], $blocks);
		//log_message('error', 'blocks:'. print_r($blocks, true));
		if ($blocks)
		{
			foreach($blocks[1] as $block => $value)
			{
						...
						<div class="halogycms_container">
							<div id="'.$blockRef.'" class="halogycms_edit">
								<div class="halogycms_buttons">
									...
									<a href="'.site_url('/admin/pages/add_block/'.$versionID.'/'.$blockRef.'/'.$page['pageID']).'" class="halogycms_savebutton"><img src="'.$this->CI->config->item('staticPath').'/images/btn_save.png" alt="Save" title="Save Changes" class="halogycms_helper" /></a>
									<a href="#" class="halogycms_editbutton">Edit</a>									
								</div>
								<div class="halogycms_blockelement">'. ( $mkdnBody && array_key_exists($blockRef, $mkdnBody)?  @$mkdnBody[$blockRef] : '').'</div>
								<div class="halogycms_editblock"><textarea rows="8" cols="10" class="code">'.( $mkdnBody && array_key_exists($blockRef, $mkdnBody)?  @$body[$blockRef]: '').'</textarea></div>
							</div>
						</div>
					';
				}
				...
				{
					$page[$blockRef] = (($mkdnBody &&array_key_exists($blockRef, $mkdnBody) )?  @$mkdnBody[$blockRef] : '');
				}				
			}
		}
	...
	function add_draft($pageID)
	{
		...
		{	
			...
			if ($blocks = $this->get_blocks($pagedata['draftID']))
			{
				...
				{
					$body = $block['body'];
					$this->add_block($body, $draftID, $block['blockRef'], $pageID);
				}
			}				
	...
	*/
	function add_block($body, $versionID, $blockRef = 'block', $pageID)
	{
	...
	*/
	function create_user()
	{
		...
		//if ($this->CI->input->post('kudos')) $this->CI->core->set['kudos'] = 0;
	...
	function captcha_check()
	{
		...
		$exp=time()-600;
		$sql = "SELECT COUNT(*) AS count FROM ps_captcha WHERE word = ? AND ip_address = ? AND captcha_time > ?";
	...
	{	
		if (count($_POST) || count($_FILES))
		{
			...
			$fields = $this->CI->db->list_fields($table);
			$fieldsMata = $this->CI->db->field_data($table);
			$row = null;
			// get data from database
			if ($id)
			{
			...
			//Edited by smy
			//Check for integer type fields, and set empty field to 0
			$numericTypes=array ('int', 'tinyint', 'double', 'smallint', 'mediumint','decimal', 'real', 'bigint', 'float', 'bit', 'boolean');
			
			foreach($fieldsMata as $fieldMata)
			{
				if( in_array( strtolower($fieldMata->type), $numericTypes) )
				{
					if ( array_key_exists($fieldMata->name, $values) && $values[$fieldMata->name] =='')
					{
						$values[$fieldMata->name]=0;
					}
				}
			}
