...
class MY_Parser extends CI_Parser {
	...
	*/
	...
	{
		//log_message('error', 'Before: '.print_r($template, true) );
		//log_message('error', 'Condition '.print_r($data, true) );
			...
			if (count($conditionals) > 0)
			{
				...
				//log_message('error',print_r($condition, true) );
					...
					if (!preg_match('/('.$this->l_delim.'|'.$this->r_delim.')/', $condString, $condProblem))
					{
						...
						{
							//log_message('error', 'Cond '.$condString );
							if (preg_match("/^!(.*)$/", $condString, $matches))
							{
								//log_message('error', '1st '.print_r($matches, true) );
								$condVar = (@!$data[trim($matches[1])]) ? 0 : $data[trim($matches[1])];

								@$result = (!$condVar) ? TRUE : FALSE;
							}
							
							elseif (preg_match("/([\s\S]+)(\!=|=|==|>|<)([\s\S]+)/", $condString, $matches))
							
							//elseif (preg_match("/([a-z0-9\-_:\(\)]+)(\!=|=|==|>|<)([a-z0-9\-_\/]+)/", $condString, $matches))
							{
								//log_message('error', '2nd '.print_r($matches, true).print_r($data, true) );
		...
		//log_message('error', 'After: ' .print_r($template, true) );
		return $template;
	}
