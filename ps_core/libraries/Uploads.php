...
class Uploads {
	...
	var $siteID;
	var $uploadsPath;
	var $imageDisplayPath; //Edited by smy
	...
	{	
		...
		$this->imageDisplayPath = $this->CI->config->item('sitePath'). $this->uploadsPath; //Edited by smy
	...
	function load_image($image, $thumbnail = false, $product = false, $category=false)
	{	
		...
		if ($product)
		{
			...
			if ($query->num_rows())
			{
				...
				$image = $row['imageName'];				
				$imagePath =( ($image==$this->CI->config->item('noPictureFile'))? $this->CI->config->item('staticFolder').'/images' :$pathToUploads ) .'/'.$image;			
				$row['src'] = $imagePath;				
			}
			else
			{
		...
		elseif ( $category) 
		{
			$query = $this->CI->db->get_where('shop_cats', array('catID' => $image, 'siteID' => $this->siteID));
			...
			if ($query->num_rows())
			{
				...
				$imagePath =empty($image) ? $this->CI->config->item('staticFolder').'/images/' .$this->CI->config->item('noPictureFile') : ( ( (  $image==$this->CI->config->item('noPictureFile')  )? $this->CI->config->item('staticFolder').'/images' :$pathToUploads ) .'/'.$image) ;			
				$row['src'] = $imagePath;				
			}
			else
			{
		...
		//log_message('error', 'Looking image: '.$image .' '.$imagePath);
		// check file exists
		...
		{
			log_message('error', 'Looking image: '.$image .' NOt found'.$imagePath);
			return FALSE;
		}
	}
