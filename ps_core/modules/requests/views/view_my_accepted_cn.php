...
<h1><strong><?php echo ($this->session->userdata('firstName')) ? ucfirst($this->session->userdata('firstName')) : $this->session->userdata('username'); ?> </strong> 已接客户单</h1>
...
<br/>
		...
		<span >
			...
			<input type="text" name="table_search" id="table_search" class="input mid-margin-left" title="搜索客户单..." placeholder="搜索客户单..." />
			</span>
			
			<a title='Refine or search' class='button icon-search with-tooltip' id="searchbutton" >搜索</a>
		...
		<table class="table responsive-table responsive-table-on dataTable" id="sorting-advanced" aria-describedby="sorting-advanced_info">
				...
				<thead>
					...
					<th scope="col" width="15%" class="align-center sorting" role="columnheader" tabindex="0" aria-controls="sorting-advanced" rowspan="1" colspan="1" aria-label="Product name" style="width: 160px;">图片</th>
					<th scope="col" width="15%" class="sorting" role="columnheader" tabindex="0" aria-controls="sorting-advanced" rowspan="1" colspan="1" aria-label="Customer" style="width: 160px;"><?php echo order_link('/admin/requests/view_my_summary','reprName','销售代表'); ?></th>
		...
		<?php if ($requests): $isOdd=TRUE; ?>
			...
			<tr><td colspan="6" rowspan="1">
					<?php echo $this->pagination->total_rows; ?> 客户单.
			</td></tr>
			</tfoot>
