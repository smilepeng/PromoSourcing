...
class Pages extends MX_Controller {
	...
	{	
		//log_message('error', 'page '.print_r($page, true).' sendthru:'.print_r($sendthru, true).' module:'.print_r($module, true));
		...
		if ($module)
		{
			...
			if ($metadata = $this->core->get_page(FALSE, substr($this->uri->uri_string(), 1)))
			{
				...
				else
				{
					...
					show_404('Not Allowed to view page '.$this->uri->uri_string());
				}
			}
			...
			if ($pagedata = $this->core->get_module_template($page))
			{
				...
				//log_message('error', 'template by name:'.$module.' template'.print_r($template, true) );
				// output data
				if ($return === FALSE)
				{
			...
			{	
				...
				{	
					...
					// set a null title
					$template['page:title'] = (!isset($sendthru['page:title'])) ? $this->site->config['siteName'] : $sendthru['page:title'];
					//log_message('error', 'from  /views/templates/'.$page.'.php'.print_r($template, true) );
					...
					//log_message('error', 'template from file:'.$module.' template'.print_r($template, true) );
				}
				else
				{
		...
		elseif ($this->session->userdata('session_admin') && $pagedata = $this->core->get_page(FALSE, $page))
		{
			...
			(!@in_array('pages_all', $this->permission->permissions) && $this->session->userdata('groupID') && $pagedata['groupID'] == $this->session->userdata('groupID')))
			{
				...
				//log_message('error', 'from '.$pagedata['pageID'].'.php'.print_r($output, true) );
			...
			else
			{
				...
				// merge output with any other data
				$output = (is_array($sendthru)) ? array_merge($output, $sendthru) : $output;
				//log_message('error', 'from normal '.$pagedata['pageID'].'.php'.print_r($output, true) );
				// parse with main cms template				
				if ($return === FALSE)
				{
					$this->parser->parse($parseFile, $output);
		...
		elseif ($pagedata = $this->core->get_active_page($page))
		{
			...
			// add view, accumulate no. of visiting
			...
			//log_message('error', 'from active page '.$pagedata['pageID'].'.php'.print_r($output, true) );
			// parse with main cms template
			if ($return === FALSE)
			{
				$this->parser->parse($parseFile, $output);
		...
		{
			show_404($page. 'is not avaiable to view!');
		}		
	}
	...
	function files($type = '', $ref = '')
	{
		...
		if ($type == 'css')
		{
			...
			{
				show_404($ref." css could not be found!");
			}
		}
		...
		elseif ($type == 'js')
		{
			...
			{
				show_404($ref." js could not be found!");
			}
		}
		...
		elseif ($type == 'images' || $type == 'gfx' | $type == 'thumbs')
		{
			...
			{
				show_404($ref.' '.$filename.' '. $type ." could not be found!");
			}
			...
			{
				show_404($ref. ' '.$filename .' '.$type ." was not uploaded!");
			}
		}
		...
		{	
			...
			{
				show_404($ref .' '. $filename . " file could not be found");
			}
		}
		...
		{
			show_404( $ref .' '. $filename . " unknown type file could not be found");
		}
	}
