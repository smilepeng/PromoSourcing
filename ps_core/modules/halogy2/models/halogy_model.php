...
class Halogy_model extends CI_Model {
	...
	*/
	...
	function get_recent_open_requests($record=10)
	{
		if( !is_int( $record) ) $record=10;
		
		$this->db->where('siteID', $this->siteID);
		$this->db->where('status', 'O');
		$this->db->order_by('dateCreated', 'desc');	
		$query = $this->db->get('requests', $record, 0);
	...
	function get_recent_accepted_requests($record=10)
	{
		if( !is_int( $record) ) $record=10;
	...
	function get_recent_archived_requests($record=10)
	{
		if( !is_int( $record) ) $record=10;
		
		$this->db->where('siteID', $this->siteID);
		$this->db->where('status', 'K');
		$this->db->order_by('dateArchived', 'desc');	
		$query = $this->db->get('requests', $record, 0);
