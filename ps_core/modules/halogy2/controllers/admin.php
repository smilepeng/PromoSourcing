...
class Admin extends MX_Controller {
	...
	function activity_ajax()
	{
		...
		if ($this->session->userdata('session_user'))
		{
			...
			$data['recentOpenRequests'] = $this->halogy->get_recent_open_requests();		
			$data['recentAcceptedRequests'] = $this->halogy->get_recent_accepted_requests();
			$data['recentCompletedRequests'] = $this->halogy->get_recent_completed_requests();
	
			$this->load->view('request_activity_ajax', $output);
		}
	}
	...
	function login($redirect = '')
	{
		...
		if (!$this->session->userdata('session_user'))
		{
			...
			{	
				...
				//$isRemember= $this->input->post('remember_me') ? TRUE  : FALSE; , FALSE, $isRemember
	...
	function backup()
	{
		...
		$filename = 'ps_backup_'.date('Y-m-d_H-i', time());
		
		// Set up our default preferences
		$prefs = array(
							'tables'		=> $this->db->list_tables(),
							'ignore'		=> array('ps_ci_sessions', 'ps_captcha', 'ps_permissions', 'ps_zipcodes'),
