...
class Shop extends MX_Controller {
	...
	function __construct()
	{
		...
		{
			//log_message('error', print_r($this->permission->sitePermissions , true));
			// get site permissions and redirect if it don't have access to this module
			if (isset($this->permission->sitePermissions) && !$this->permission->sitePermissions) //Edited by smy sitePermissions -> permissions
			{
	...
	function categories($parentID=0) //By default, get first level of categories whose parent ID =0 
	{
	
		if ( $categories=$this->shop->get_category_parents($parentID) )
		{
			 $this->output->set_output($this->_populate_categories_html($categories)  );
	...
	function browse($cat = '', $parent = '')
	{
		...
		if ($category)
		{
			...
			// get products
			/*
			$output['shop:categories'] = null;
			if ($categories = $this->shop->get_categories_by_parentID(0))
			{
				// load content
				$output['shop:categories'] = $this->_populate_categories($categories);
			}
			*/
			//log_message('error', 'cat: '.$cat .' parent '.$parent );
			//log_message('error', 'Products: '.print_r($output, true).' category: '. print_r($category, true) );
			// populate template
			$output['rowpad'] = '';
			for ($x = 0; $x < ($this->shop->siteVars['shopItemsPerRow'] - sizeof($products)); $x++)
			{
			...
			$output['category:img'] = $this->config->item('staticFolder').'/images/nopicture.jpg';;
			...
			$output['page:heading'] =$category['catName'];
	...
	function viewproduct($productID)
	{
		...
		$output['product:image-path'] = $this->config->item('sitePath').$image['src'];

		$image = $this->uploads->load_image($productID, true, true);
		$output['product:thumb-path'] = $this->config->item('sitePath').$image['src'];
		...
		$title =(strlen($this->_strip_markdown($product['productName'])) > 20) ? substr($this->_strip_markdown($product['productName']), 0, 20).'...' : nl2br($this->_strip_markdown($product['productName'])); 
	
		$output['product:title'] = $title;
		$output['product:title-full'] =$product['productName'];	
		...
		$output['product:massageIsAvailable'] = ($output['product:category'] =='Massage' && $product['stock'])?  1 :0;
		$output['product:productIsAvailable'] = ($output['product:category'] !='Massage' && $product['stock'])?  1 :0;
		// get tags
		if ($product['tags'])
		{
		...
		//Set up review pagination
		 $config['per_page'] = 10; 
			...
			$config['full_tag_open'] = '<div class="pagination"><ul>';
			$config['full_tag_close'] = '</ul></div>';
			$config['base_url'] = 'javascript:load_review('.$product['productID'].', %d)';
			$this->pagination->initialize($config); 
		...
		if ($reviews = $this->shop->get_reviews($product['productID']))
		{
		 ...
		 $output['pagination'] = ($pagination = $this->pagination->create_java_links()) ? $pagination : '';
		// set status
		if ($product['status'] == 'S')
		{
	...
	//$quantity is the $action, modifed by Smy 
	...
	{	
		...
		{	
			if ($action == 'add' && $this->input->post('productID'))
			{
		...
		$output['upsell:image-path'] = ($image) ? $this->config->item('sitePath').$image['src'] : $this->config->item('staticPath').'/images/nopicture.jpg';
		$image = ($upsell) ? $this->uploads->load_image($upsell['productID'], true, true) : '';
		$output['upsell:thumb-path'] = ($image) ? $this->config->item('sitePath').$image['src'] : $this->config->item('staticPath').'/images/nopicture.jpg';
	...
	//$quantity is the $action, modifed by Smy 
	function appointment($action = '', $id = '')
	{	
		// get partials
		$output = $this->partials;
		...
		if ($action == 'add' && $this->input->post('productID'))
		{
			...
			$this->shop->add_to_appointment($this->input->post('productID'), $this->input->post('app_date_time'));	
		}
		...
		// redirects
		if ($this->input->post('checkout_appointment'))
		{
			redirect('/shop/checkout_appointment');
		}
		...
		// set page title
		$output['page:title'] = 'Making Appointment '.(($this->site->config['siteName']) ? ' - '.$this->site->config['siteName'] : '');

		if (!$this->core->is_ajax())
		{
			// display with cms layer	
			$this->pages->view('shop_appointment', $output, TRUE);
		}
		
	}
	
	function checkout_appointment()
	{
		// get partials
		$output = $this->partials;
		...
		if (@$user['country'] == 'AU' && !@$user['postcode'])
		{
		...
		// check a state is set up
		if (@$user['billingCountry'] == 'AU' && !@$user['billingState'])
		{
		...
		// check a zipcode is set up
		if (@$user['billingCountry'] == 'AU' && !@$user['billingPostcode'])
		{
		...
		if ($data = $this->shop->load_appointment())
		{
			// get transaction data
			$transaction = $this->shop->insert_transaction();
			
			// populate template
			$output['appointment:discounts'] = ($data['discounts'] > 0) ? currency_symbol().number_format(@$data['discounts'], 2) : '';
			$output['appointment:subtotal'] = currency_symbol().number_format($data['subtotal'], 2);
			$output['appointment:postage'] = currency_symbol().number_format($data['postage'], 2);
			$output['appointment:tax'] = ($data['tax'] > 0) ? currency_symbol().number_format($data['tax'], 2) : '';
			$output['appointment:total'] = currency_symbol().number_format(($data['subtotal'] + $data['postage'] + $data['tax']), 2);
			$output['appointment:amount'] = number_format(($data['subtotal'] + $data['postage'] + $data['tax']), 2);
			...
			$data['user'] = $user;
			$data['subtotal'] = $output['appointment:subtotal'];
			$data['amount'] = $output['appointment:amount'];
	...
	function create_account($redirect = '')
	{
		...
		$output['select:billingState'] = @display_states('billingState', (array_key_exists('billingState', $data))?$data['billingState']:'', 'id="billingState" class="formelement"');
	...
	function ipn()
	{
		log_message('error', 'ipn entry:'.print_r($_POST,true));
		...
		if ($this->shop->validate_ipn())
		{
			if ($this->shop->validate_payment())
			{
				...
				elseif (substr($this->shop->response_data['orderID'], 0, 3) == 'OPT')
				{
					// send donation email
					$this->_create_oppointment();
				}
	...
	function _create_appointment($appointmentID = '')
	{
		// get order ID
		$appointmentID = ($appointmentID) ? $appointmentID : $this->shop->response_data['orderID'];

		// get order details
		$orderRow = $this->shop->get_appointment_by_appointment_id($appointmentID);
		$transactionID = $orderRow['transactionID'];		

		// get ordered products
		$appointmentDetail = $this->shop->get_appointment_detail($transactionID);
		...
		// construct email to admin
		$adminBody = "Dear administrator,\n\n";
		$adminBody .= "An appointment (#".$appointmentID.") has been placed on ".$this->site->config['siteName'].".\n\n";
		$adminBody .= "------------------------------------------\n";
	
		// grab order and make body
		$orderBody = "Your appointment:\n\n";
		$orderBody .= "Reference ID #: ".$appointmentID."\n\n";
		...
		$downloadBody = '';
			...
			if ($this->site->config['shopStockControl'])
			{
				$this->shop->minus_stock($appointmentDetail['productID'], $appointmentDetail['quantity']);
			}
		
			
			$orderBody .= $appointmentDetail['quantity'] . "x | #" . $appointmentDetail['catalogueID'] . " | " . $appointmentDetail['productName']. " ";
			$orderBody .= "| ".currency_symbol(FALSE). number_format(($appointmentDetail['price'] ),2)."\n";
		...
		$this->shop->update_appointment($transactionID);

		// load email lib and email user and admin
		$this->load->library('email');

		$this->email->to($orderRow['email']);
		$this->email->subject('Thank you for your order (#'.$appointmentID.')');
		$this->email->message($userBody.$orderBody.$dispatchBody.$notesBody.$footerBody);
		$this->email->from($this->site->config['siteEmail'], $this->site->config['siteName']);			
		$this->email->send();
		...
		$this->email->to($this->site->config['siteEmail']);
		$this->email->subject('Someone has placed an order (#'.$appointmentID.')');
		$this->email->message($adminBody.$orderBody.$dispatchBody.$notesBody.$footerBody);
		$this->email->from($this->site->config['siteEmail'], $this->site->config['siteName']);			
		$this->email->send();
	...
	function _populate_products($products)
	{
		if ($products && is_array($products))
		{
			...
			foreach ($products as $product)
			{
				...
				{
					$image['src'] = $this->config->item('staticFolder').'/images/nopicture.jpg';
				}
				if (!$thumb = $this->uploads->load_image($product['productID'], true, true))
				{
					$thumb['src'] = $this->config->item('staticFolder').'/images/nopicture.jpg';
				}
				...
				$data[$x] = array(
					...
					'product:title-full' => $product['productName'],
					'product:subtitle' => $product['subtitle'],
					'product:body' => $productBody,
					'product:excerpt' => $productExcerpt,
					'product:image-path' =>	$this->config->item('sitePath').$image['src'],				
					'product:thumb-path' => $this->config->item('sitePath').$thumb['src'],
	...
	function _populate_categories($categories)
	{
		if ($categories && is_array($categories))
		{
			...
			{
				$description=(strlen($this->_strip_markdown($category['description'])) > 100) ? substr($this->_strip_markdown($category['description']), 0, 100).'...' : nl2br($this->_strip_markdown($category['description']));		
				
				// get images
				$image='';
				if (!$image = $this->uploads->load_image($category['catID'], false, true))
				{
					$image['src'] = $this->config->item('staticFolder').'/images/nopicture.jpg';
				}
				if (!$thumb = $this->uploads->load_image($category['catID'], true, true))
				{
					$thumb['src'] = $this->config->item('staticFolder').'/images/nopicture.jpg';
				}
				...
				$data[$x] = array(
					...
					'category:link' => '/shop/categories/'.$category['catID'],
					'category:title' => $category['catName'],
					'category:description' => $description,
					'category:image-path' =>	$this->config->item('sitePath').$image['src'],				
					'category:thumb-path' => $this->config->item('sitePath').$thumb['src'],
					'category:cell-width' => floor(( 1 / $itemsPerRow) * 100)
				);
				...
				if (($i % $itemsPerRow) == 0 && $i > 1)
				{
					$data[$x]['category:rowpad'] = '</tr><tr>'."\n";
					$i = 0;
				}
				else
				{
					$data[$x]['category:rowpad'] = '';
				}
	...
	function _populate_categories_html($categories)
	{
		if ($categories && is_array($categories))
		{
			...
			foreach ($categories as $category)
			{
				...
				$link=	site_url('shop/browse/'.$category['catID']);
				// get body 
								$data .= ' <div class="grid_4"> <a href="'.$link.'">
						<div class="box1">
						  <div class="imgs"> <img src="'.$this->config->item('sitePath').$image['src'].'" alt="'.$category['description'].'" class="img-1" />  <img src="'.$this->config->item('sitePath').$image['src'].'" alt="'.$category['description'].'"  class="img-2" /> </div>
