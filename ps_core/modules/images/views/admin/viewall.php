...
$(function(){
	...
	$('a.lightbox').lightBox({imageLoading:'<?php echo $this->config->item('staticPath'); ?>/images/loading.gif',imageBtnClose: '<?php echo $this->config->item('staticPath'); ?>/images/lightbox_close.gif',imageBtnNext:'<?php echo $this->config->item('staticPath'); ?>/images/lightbox_btn_next.gif',imageBtnPrev:'<?php echo $this->config->item('staticPath'); ?>/images/lightbox_btn_prev.gif'});
...
<div id="upload-image" class="hidden clear">
	<form method="post" action="<?php echo site_url($this->uri->uri_string()); ?>" enctype="multipart/form-data" class="default">
		...
		<?php /*log_message('error', 'Images'.print_r($images, true));*/ 
		// echo @form_input('imageName', empty($images)?'':$images[0]['imageName'], 'class="formelement" id="imageName"');
			echo @form_input('imageName', '', 'class="formelement" id="imageName"'); 		?>
		<br class="clear" />
		...
		<?php
			...
			echo @form_dropdown('folderID',$options,set_value('folderID', $folderID),'id="imageFolderID" class="formelement"'); //Edited by smy $data['folderID']
		?>	
		<br class="clear" /><br />	
...
<div id="upload-zip" class="hidden clear">
	<form method="post" action="<?php echo site_url($this->uri->uri_string()); ?>" enctype="multipart/form-data" class="default">
		...
		<?php
			...
			echo @form_dropdown('folderID',$options,set_value('folderID', $folderID),'id="zipFolderID" class="formelement"'); //Edited by smy
		?>
		<br class="clear" /><br />		
	...
	<table class="images clear">	
		...
		<?php
			...
			foreach ($images as $image)
			{
				...
				$sitePath=$this->config->item('sitePath');
		?>
				<div class="buttons">
				...
				<a href="<?php echo $sitePath.$imagePath; ?>" title="<?php echo $image['imageName']; ?>" class="lightbox"><?php echo ($thumb = display_image($imageThumbPath, $image['imageName'], 100, 'class="pic"', FALSE, $sitePath)) ? $thumb : display_image($imagePath, $image['imageName'], 100, 'class="pic"', FALSE, $sitePath); ?></a>
