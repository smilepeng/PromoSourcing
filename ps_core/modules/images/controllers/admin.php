...
class Admin extends CI_Controller {
	...
	function __construct()
	{
		...
		//$this->selections = (is_array($selections)) ? @$selections[$this->uri->segment(2)] : '';  //Edited Smy
	...
	function viewall($folderID = '')
	{
		...
		{			
			...
			elseif ($oldFileName = @$_FILES['image']['name'])
			{
				...
				$imageRef = url_title(trim(substr(strtolower($imageName),0,99)));
		...
		$output['images']=null;  //Edited by smy
		$limit= $this->site->config['paging']; //Added by smy
		//log_message('error', 'config:'.print_r($this->site->config['paging'], true));
		// search
		if ($this->input->post('searchbox'))
		{
			$output['images'] = $this->images->search_images($this->input->post('searchbox'), $limit);
		}
		...
		else
		{
			...
			if ($folderID == '' && is_array($this->selections) && @array_key_exists('folderID', $this->selections)) //Edited by smy
			{
		...
		//log_message('error', 'view all image by search "'. $this->input->post('searchbox'). '" Found: '. print_r($output, true));
	...
	function edit($imageID, $redirect = '', $popup = FALSE)
	{
		...
		{
			// set image reference and only add to db if its unique
			$imageRef = url_title(trim(substr(strtolower($this->input->post('imageRef')),0,99)));
	...
	function browser()
	{
		...
		$output['folders'] = null; //Added by smy
		// get folders
	...
	{	
		...
		$limit= $this->site->config['paging'];				//Added by smy
		// form dropdown
		$results = $this->images->search_images($q, $limit); //Edited by smy
		$items=null;
		// go foreach
		...
		//log_message('error', 'Search: '. $q.'Found result: '.print_r($results, true) .' output: '.print_r($output, true)  );
