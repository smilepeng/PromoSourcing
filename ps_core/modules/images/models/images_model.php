...
class Images_model extends CI_Model {
	...
	function search_images($query, $limit = 20) //Edited by smy ($limit='') by default nothing will return due to limit=0
	{
		if (!$query)
		{
			return FALSE;
		...
		//log_message('error', 'search image db:'. print_r($this->db, true));
		$images = $this->db->get('images', $limit);
		//log_message('error', 'search images:'. print_r($images, true));
		if ($images->num_rows())
		{
